...
select rooms.*,
  ...
  (select is_on      from smart_plugs where room_id = rooms.room_id order by created_at desc limit 1),
  (select power      from smart_plugs where room_id = rooms.room_id order by created_at desc limit 1)
